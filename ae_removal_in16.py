...
trainset = ImageNet16(root='/home/lucky/datasets/cifar.python/ImageNet16', train=True, transform=transform, use_num_of_class_only=120)
valset = ImageNet16(root='/home/lucky/datasets/cifar.python/ImageNet16', train=False, transform=transform, use_num_of_class_only=120)
print(len(trainset))
...
def autoencoder_training(epochs, train_set, val_set):
    ...
    val_loader = torch.utils.data.DataLoader(val_set, batch_size=128,
                                              shuffle=False)
    os.makedirs('./imagenet16_outputAE/', exist_ok=True)
    model = Autoencoder()
    ...
    for epoch in range(epochs):
        ...
        torch.save(model.cpu(), "./imagenet16_outputAE/ae_%d" % (epoch + 1))
        model.to(device)

        vutils.save_image(output_decoded[:16], './imagenet16_outputAE/fake_%d.pdf' % (epoch + 1))
        if epoch == 0:
            vutils.save_image(inputs[:16], './imagenet16_outputAE/real.pdf')

    import pickle
    with open('./imagenet16_outputAE/train_losses.pkl', 'wb') as f:
        pickle.dump(train_losses, f)
    with open('./imagenet16_outputAE/val_losses.pkl', 'wb') as f:
    ...
    plt.savefig("./imagenet16_outputAE/loss_curves.pdf")
    plt.show()


def ae_removal(train_loader, fraction, epoch):
    print("Konverting to numpy arrays...")
    model = torch.load("./imagenet16_outputAE/ae_%d" % epoch)
    ...
    with open('/home/lucky/datasets/cifar.python/imagenet16_AE_easy_' + str(fraction) + '.pkl', 'wb') as f:
        pickle.dump(result, f)
    result = losses.argsort()[::-1][:int(len(train_loader) * fraction)].tolist()
    with open('/home/lucky/datasets/cifar.python/imagenet16_AE_hard_' + str(fraction) + '.pkl', 'wb') as f:
        pickle.dump(result, f)
...
if __name__ == '__main__':
    ...
    ae_removal(trainloader, 0.75, 10)
    ae_removal(trainloader, 0.50, 10)
    ae_removal(trainloader, 0.25, 10)
