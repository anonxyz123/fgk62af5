...
def main(xargs):
  ...
  _, train_loader, valid_loader = get_nas_search_loaders(train_data, test_data, xargs.dataset, 'configs/nas-benchmark/', config.batch_size, xargs.workers, args.sample_mode, args.sample_fraction)
  # since ENAS will train the controller on valid-loader, we need to use train transformation for valid-loader
  valid_loader.dataset.transform = deepcopy(train_data.transform)
  if hasattr(valid_loader.dataset, 'transforms'):
    valid_loader.dataset.transforms = deepcopy(train_data.transforms)
  # data loader
...
if __name__ == '__main__':
  ...
  parser.add_argument('--sample_mode',        type=str,   help='sampling mode')
  parser.add_argument('--sample_fraction',    type=float, help='sampling fraction')
